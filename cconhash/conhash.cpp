...
void conhash_set_node(struct node_s *node, int index, const char *iden, u_int replica)
{
...
const struct node_s* conhash_lookup(struct conhash_s *conhash, const char *object,int len)
{
    long hash;
    const util_rbtree_node_t *rbnode = NULL;
...
char* conhash_lookup_go(struct conhash_s *conhash, const char *object,int len)
{
    const struct node_s *node = conhash_lookup(conhash, object, len);
    return const_cast<char *>(node->iden);
}
