 ...
 * @instr: input string
 */
typedef long (*conhash_cb_hashfunc)(const char *instr,int len);
...
extern "C" {
    ...
    /* set node */
    CONHASH_API void conhash_set_node(struct node_s *node, int index, const char *iden, u_int replica);
     ...
     */
	CONHASH_API const struct node_s* conhash_lookup(struct conhash_s *conhash, const char *object,int len);
	CONHASH_API char* conhash_lookup_go(struct conhash_s *conhash, const char *object,int len);

    /* some utility functions export*/    
    CONHASH_API void  conhash_md5_digest(const u_char *instr,int len, u_char digest[16]); 
    ...
    */
    CONHASH_API void  conhash_get_vnodes(struct conhash_s *conhash, long *values, int size);
